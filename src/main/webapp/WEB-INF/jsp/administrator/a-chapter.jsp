<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>课程列表页</title>
    <link rel="stylesheet" href="../css/bootstrap.min.css">
    <link rel="stylesheet" href="../css/fontAwesome/font-awesome.min.css">
    <link rel="stylesheet" href="../css/base.css">
    <link rel="stylesheet" href="../css/m-index.css">
    <script src="../js/jquery.min.js"></script>
    <script src="../js/bootstrap.min.js"></script>
</head>
<body>

<div class="m-header">
    <div class="logo-field">
        <a href="#"><img src="../images/logo.png" alt=""></a>
    </div>
    <ul>
        <li><a href="${pageContext.request.contextPath}/administrator/a-loginout.html"><i class="fa fa-power-off"></i>退出登录</a></li>
        <li><a href="#">欢迎来到后台管理系统</a></li>
    </ul>
</div>

<div class="m-sidebar">
    <ul>
        <li><a href="${pageContext.request.contextPath}/administrator/a-index.html?page=1&leixing=1"><i class="fa fa-home"></i>首页</a></li>
        <li><a href="${pageContext.request.contextPath}/administrator/a-unicepage.html?page=1&leixing=1"><i class="fa fa-th-large"></i>用户解冻</a></li>
        <li><a href="${pageContext.request.contextPath}/administrator/a-teacher.html"><i class="fa fa-dashboard"></i>讲师管理</a></li>
        <li><a href="${pageContext.request.contextPath}/administrator/a-class.html"><i class="fa fa-folder-open"></i>课程管理</a></li>
    </ul>
</div>

<div class="m-content">
    <!-- 路径导航 -->
    <ol class="breadcrumb">
        <li><i class="fa fa-flag"></i></li>
        <li><a href="${pageContext.request.contextPath}/administrator/a-index.html?page=1&leixing=1">首页</a></li>
        <li><a href="${pageContext.request.contextPath}/administrator/a-class.html">课程管理</a></li>
        <li class="active"><c:out value="${ke}"></c:out></li>
    </ol>
    <!-- /路径导航 -->
    <div class="control-bar">
        <span><c:out value="${ke}"></c:out></span>
        <form class="form-inline">
            <div class="input-group">
                <input type="text" class="form-control" name="" placeholder="按名称搜索">
                <div class="input-group-addon"><i class="fa fa-search"></i></div>
            </div>
        </form>
    </div>

    <!-- 内容部分 -->
    <div class="m-content-field">
        <table class="table">
            <thead>
            <tr>
                <th>章节id</th>
                <th>章节名</th>
                <th>第几章</th>
                <th>学习人数</th>
                <th>审核状态</th>
                <th>审核操作</th>
                <th>删除课程</th>
            </tr>
            </thead>
            <c:forEach var="userinfo" items="${userinfos}">
                <tr>
                    <td><c:out value="${userinfo.id}"></c:out></td>
                    <td><c:out value="${userinfo.name}"></c:out></td>
                    <td>第<c:out value="${userinfo.number}"></c:out>章</td>
                    <td><c:out value="${userinfo.study}"></c:out></td>
                    <td><c:out value="${userinfo.stauszhuangtai}"></c:out></td>
                    <td>
                        <c:if test="${userinfo.status == 0}"><a href="${pageContext.request.contextPath}/administrator/a-chapterpass.html?pass=1&id=<c:out value="${userinfo.id}"></c:out>&cid=<c:out value="${classid}"></c:out>" class="btn btn-xs btn-info">通过审核</a>/<a href="${pageContext.request.contextPath}/administrator/a-chapterpass.html?pass=0&id=<c:out value="${userinfo.id}"></c:out>&cid=<c:out value="${classid}"></c:out>" class="btn btn-danger btn-xs">不通过审核</a></c:if>
                        <c:if test="${userinfo.status == 1}"><a href="${pageContext.request.contextPath}/administrator/a-chapterpass.html?pass=0&id=<c:out value="${userinfo.id}"></c:out>&cid=<c:out value="${classid}"></c:out>" class="btn btn-danger btn-xs">不通过审核</a></c:if>
                        <c:if test="${userinfo.status == 2}"><a href="${pageContext.request.contextPath}/administrator/a-chapterpass.html?pass=1&id=<c:out value="${userinfo.id}"></c:out>&cid=<c:out value="${classid}"></c:out>" class="btn btn-xs btn-info">通过审核</a></c:if>
                    </td>
                    <td>
                        <a class="btn btn-danger btn-xs" href="${pageContext.request.contextPath}/administrator/a-dechapter.html?id=<c:out value="${userinfo.id}"></c:out>&cid=<c:out value="${classid}"></c:out>">删除课程</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
    </div>

    <!-- /内容部分 -->
    <nav style="text-align:center;">
        <ul class="pagination">
            <li>
                <a href="${pageContext.request.contextPath}/administrator/a-chapterpage.html?page=<c:out value="${now}"></c:out>&leixing=3&id=<c:out value="${classid}"></c:out>" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            <c:forEach var="page" items="${pages}">
                <li ><a href="${pageContext.request.contextPath}/administrator/a-chapterpage.html?page=<c:out value="${page}"></c:out>&leixing=1&id=<c:out value="${classid}"></c:out>"><c:out value="${page}"></c:out></a></li>
            </c:forEach>

            <li>
                <a href="${pageContext.request.contextPath}/administrator/a-chapterpage.html?page=<c:out value="${now}"></c:out>&leixing=2&id=<c:out value="${classid}"></c:out>" aria-label="Next">
                    <span aria-hidden="true">&raquo;</span>
                </a>
            </li>

            <li>
                第<c:out value="${now}"></c:out>页
            </li>


        </ul>
    </nav>
</div>

</body>
</html>